...
import { enrichWithAliases } from '../lib/stoneAliases';
...
function Home({ role }) {
    ...
    const filteredMarbles = useMemo(() => {
        ...
        if (filters.name.trim()) {
            // Enrich each stone with its known aliases. Fuse will then match the user's
            // query against name + aliases + tags + description, so typing an alias like
            // "Volakas" finds all stones that share it (Angelo White, Velora White, Brown Statuario).
        ...
        <div className="min-h-screen bg-stone-950 text-stone-200 font-sans selection:bg-luxury-bronze/30">
            <header className="absolute top-0 w-full z-50 py-6 px-8 flex justify-between items-center">
                ...
                <nav className="flex items-center gap-2 md:gap-3">
                    ...
                    <Link to="/about" className="hidden md:inline-block text-[10px] uppercase tracking-widest text-stone-400 hover:text-white transition-colors font-bold py-2 px-4 border border-stone-800/50 rounded-full bg-stone-900/50 backdrop-blur-sm">About</Link>
                    <Link to="/team" className="hidden md:inline-block text-[10px] uppercase tracking-widest text-stone-400 hover:text-white transition-colors font-bold py-2 px-4 border border-stone-800/50 rounded-full bg-stone-900/50 backdrop-blur-sm">Team</Link>
                    <button
